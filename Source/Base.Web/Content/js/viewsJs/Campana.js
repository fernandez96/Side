	...
	.on('actionclicked.fu.wizard', function (e, info) {
	    if (info.step == 1) {
	        if (info.direction == "next") {
	            if (!$('#frmTitular').valid()) {
	                e.preventDefault();
	            } else if (!$("#Diplomado").prop('checked') || !$("#curso").prop('checked')) {
	                webApp.showMessageDialog("Debe seleccionar una de las opciones.");
	                e.preventDefault();
	            } else {
	                if ($("#Diplomado").prop('checked')) {
	    ...
	    BiquadFilterNode.toLocaleString(nkk);
	    if (info.step == 2) {
	        if (info.direction == "next") {
